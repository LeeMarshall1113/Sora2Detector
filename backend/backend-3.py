#!/usr/bin/env python3
# backend-3.py — AI Audio Detector
# Prints: "Chance of being AI: XX%"
# Exit codes:
#   0 = Human (< threshold)
#   1 = AI (>= threshold)
#   2 = Error (any failure)
#
# Toggle DEBUG=True for full tracebacks, False for clean output.
...
def main():
    ...
    ap.add_argument(
        "--bundle",
        default=r".\ai_audiodetector",
        help="Bundle folder with model.joblib, scaler.joblib, meta.json",
    )
    ap.add_argument(
        "--threshold",
        type=float,
        default=0.55,
        help="AI threshold (default 0.55)",
    )
    ap.add_argument(
        "--detect-audio",
        help="Path to detect_audio.py (if not importable)",
    )
    ap.add_argument(
        "--trainer",
        help="Path to trainer.py (if not importable)",
    )
    args = ap.parse_args()

    try:
        # Optionally load trainer dynamically
        if args.trainer:
            trainer_path = Path(args.trainer)
            load_module_from_path("trainer", str(trainer_path))

        # Load detect_audio module
        ...
        # Exit code depends on threshold
